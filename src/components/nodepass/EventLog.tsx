...
export function EventLog({ apiId, apiName, apiRoot, apiToken }: EventLogProps) {
  ...
  const [selectedEventTypes, setSelectedEventTypes] = useState<Set<InstanceEvent['type']>>(new Set(ALL_EVENT_TYPES));
  const [selectedLogLevels, setSelectedLogLevels] = useState<Set<string>>(new Set(ALL_LOG_LEVELS));
  ...
  const processSseMessageData = useCallback((messageBlock: string) => {
    let eventTypeFromServer = 'message'; // Default SSE event type if not specified
    let eventDataLine = '';
    ...
    if (eventTypeFromServer === 'instance' && eventDataLine) {
      try {
        ...
        let frontendEventData: any = serverEventPayload; // Default to full payload
        ...
        switch (serverEventPayload.type) {
          ...
          case 'shutdown':
            ...
            if (abortControllerRef.current && !abortControllerRef.current.signal.aborted) {
              abortControllerRef.current.abort("服务器关闭事件");
            }
            break;
          case 'error': // For server-sent error events
            ...
            break;
          default: // Fallback for unknown types from server
            ...
            instanceDetailsPayload = serverEventPayload.instance; // Attempt to get instance details
            break;
        }
        ...
        // Handle generic messages not specifically typed as 'instance'
        const genericEvent: InstanceEvent = {
          id: `generic-msg-${Date.now()}`,
          type: 'log', // Treat as a log
  ...
  const scheduleReconnect = useCallback((reason: string) => {
    if (!isMountedRef.current || !apiId) return;
    ...
    } else if (reason.includes("HTTP错误")) {
      uiErrorMessageForLog = `无法建立 SSE 连接 (fetch)。目标: ${eventsUrl}. 原因: ${reason}. 查看服务器日志了解详情。`;
    } else {
      uiErrorMessageForLog = `无法建立 SSE 连接 (fetch)。目标: ${eventsUrl}. 原因: ${reason}. 查看服务器日志及网络。`;
    }
    ...
    reconnectTimeoutRef.current = setTimeout(() => {
      if (isMountedRef.current && currentApiIdRef.current === apiId) {
         connectWithFetch();
      } else if (isMountedRef.current) {
        setUiConnectionStatus('idle');
      }
    }, RECONNECT_DELAY_MS);
  }, [apiId, apiRoot, addEventToLog]); // connectWithFetch is not needed here as it's called in setTimeout
  ...
  const connectWithFetch = useCallback(async () => {
    ...
    const eventsUrl = getEventsUrl(apiRoot); // This is the direct NodePass events URL
    if (!eventsUrl) {
    ...
    // Log initial "connecting" message only once per API config change
    if (retryCountRef.current === 0 && currentApiIdRef.current === apiId) {
       setEvents(prev => [{ type: 'log', data: `正在初始化事件流 (fetch) 到 ${eventsUrl} (携带 X-API-Key)...`, timestamp: new Date().toISOString() } as InstanceEvent, ...prev.filter(e => typeof e.data === 'string' && !e.data.startsWith('正在初始化') && !e.data.includes('错误') && !e.data.includes('已连接') && !e.data.includes('已禁用'))]);
    }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(`HTTP错误: ${response.status} ${response.statusText}. ${errorBody.substring(0, 200)}`);
      }
      ...
      if (!hasLoggedSuccessfulConnectionRef.current && currentApiIdRef.current === apiId) {
        addEventToLog({ id: `connected-${Date.now()}`, type: 'log', data: `事件流 (fetch) 已连接。等待事件... (目标: ${eventsUrl})`, timestamp: new Date().toISOString() } as InstanceEvent);
      ...
      while (true) {
        ...
        if (signal.aborted) break; // Check again after read promise resolves
        if (done) {
          if (!signal.aborted && isMountedRef.current) { // If stream closed by server and not by client abort
        ...
        buffer = messageBlocks.pop() || ''; // Keep incomplete message in buffer
      ...
      if (signal.aborted) {
        if (isMountedRef.current && signal.reason && 
           ...
           detailedErrorMessage = `事件流连接已中止: ${signal.reason}`;
           addEventToLog({ id: `log-abort-${Date.now()}`, type: 'log', data: detailedErrorMessage, timestamp: new Date().toISOString(), level: 'WARN' });
        }
        if (signal.reason === "组件卸载或依赖项更改"){
             setUiConnectionStatus('cancelled');
        } else if (isMountedRef.current && !signal.reason?.includes("正在计划重连")) {
            // Do not schedule reconnect if it was a planned abort for reconnect
        }
      } else if (isMountedRef.current) {
        setUiConnectionStatus('error');
        if (String(error.message).toLowerCase().includes('failed to fetch')) {
            detailedErrorMessage = `SSE 连接 (fetch) 尝试失败. 目标: ${eventsUrl}. 原因: network error. 查看服务器日志了解详情。`;
        } else if (String(error.message).includes('HTTP错误')) {
             detailedErrorMessage = `HTTP错误: ${error.message}. 请检查服务器日志和API令牌是否有效。`;
        } else {
           detailedErrorMessage = `SSE 连接错误。目标: ${eventsUrl}. 原因: ${error.message || "未知客户端错误"}.`;
        }
  ...
  }, [apiId, apiRoot, apiToken, apiName, addEventToLog, processSseMessageData, scheduleReconnect]);
  ...
  useEffect(() => {
    ...
    if (apiId && apiRoot && apiToken && apiName) {
      if (currentApiIdRef.current !== apiId) { // API config changed
        ...
        // If connection was previously in a failed/idle state for the *same* API, and no reconnect is scheduled, try connecting.
        if (!reconnectTimeoutRef.current) {
            if (uiConnectionStatus === 'cancelled') retryCountRef.current = 0; // Reset retries if manually cancelled
            connectWithFetch();
        }
      }
    } else { // No active API config
      setEvents([]);
      setUiConnectionStatus('idle');
      if (abortControllerRef.current && !abortControllerRef.current.signal.aborted) {
  ...
  // scheduleReconnect is intentionally omitted as it creates a loop with connectWithFetch
  ...
  const handleClearFilters = () => {
    setSelectedEventTypes(new Set(ALL_EVENT_TYPES));
    setSelectedLogLevels(new Set(ALL_LOG_LEVELS));
  };
    ...
    <>
      <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4 mb-4">
        <div className="flex-grow">
            {/* Title and Description are now part of the parent Card */}
        </div>
        <div className="flex items-center gap-2 flex-wrap sm:flex-nowrap justify-end">
            ...
            <DropdownMenuTrigger asChild>
                ...
                事件类型 ({selectedEventTypes.size === ALL_EVENT_TYPES.length ? '全部' : selectedEventTypes.size})
                </Button>
            </DropdownMenuTrigger>
            ...
            <DropdownMenuTrigger asChild>
                <Button variant="outline" size="sm" className="font-sans">
                <Filter className="mr-2 h-4 w-4" />
                日志级别 ({selectedLogLevels.size === ALL_LOG_LEVELS.length ? '全部' : selectedLogLevels.size})
                </Button>
            </DropdownMenuTrigger>
            <DropdownMenuContent className="w-56 font-sans">
            ...
            {(selectedEventTypes.size < ALL_EVENT_TYPES.length || selectedLogLevels.size < ALL_LOG_LEVELS.length) && (
      ...
      <div className="flex items-center text-sm text-muted-foreground mb-3 font-sans">
          ...
          {apiName && <span className="ml-2"> (主控: {apiName})</span>}
      </div>
        ...
        {filteredEvents.map((event, index) => {
          ...
          let mainContent;
            if (instance) {
                mainContent = (
                    <div className="flex flex-wrap items-center gap-x-2 gap-y-0.5 text-sm leading-tight">
                        ...
                        <div className="flex items-center text-xs">
                            ...
                            <span className="font-mono truncate" title={instance.url}>
                ...
                mainContent = <p className="text-sm text-muted-foreground italic font-sans">未知事件数据格式</p>;
            }
            ...
            <div key={event.id || `${event.timestamp}-${index}-${event.type}`} className="py-1.5 border-b border-border/30 last:border-b-0 last:pb-0 first:pt-0">
              ...
              {isExpanded && canExpand && (
                <div className="mt-2 ml-8 pl-4 border-l-2 border-muted/50 py-2 bg-background/30 rounded-r-md">
                  ...
                  ) : typeof event.data === 'string' && event.data.length > LOG_LINE_TRUNCATE_LENGTH ? (
